...
// Pastikan file produk.json ada dan valid
if (!fs.existsSync(produkFilePath)) {
  fs.writeFileSync(produkFilePath, JSON.stringify([])); // Buat file kosong jika tidak ada
}

function getProdukList() {
  try {
    const rawData = fs.readFileSync(produkFilePath, "utf-8");
    return JSON.parse(rawData); // Parse data JSON
  } catch (err) {
    console.error("❌ Gagal membaca file produk.json:", err);
...
╰────────────────────────────╯`,
  ...
  case "/produk":
  try {
    const produkList = getProdukList(); // Pastikan data produk diambil
    ...
    await reply("⚠️ Terjadi kesalahan saat memuat daftar produk. Silakan coba lagi nanti.");
  }
  break;
    case "/pembayaran":
  await delay(900);
...
case "/bukti":
  ...
  break;
    ...
    const produkList = getProdukList(); // Ambil daftar produk
    produkList. forEach((produk) => {
...
📖 *Panduan Memesan di ${BOT_NAME}*:

1. *Lihat Produk*:
   Gunakan perintah */produk* untuk melihat daftar produk yang tersedia.

2. *Pilih Produk*:
   Catat ID produk yang ingin kamu beli dari daftar produk.

3. *Format Pemesanan*:
   Gunakan perintah */pemesanan* untuk memulai proses pemesanan. Isi format yang diberikan dengan detail berikut:
   - ID Produk (contoh: 1,2 jika membeli lebih dari satu produk)
   - Jumlah (sesuai dengan ID produk yang dipilih)
   - Alamat Pengiriman
   - Metode Pembayaran

4. *Kirim Bukti Pembayaran*:
   Setelah transfer, kirim bukti pembayaran berupa foto melalui bot dengan perintah */bukti*.

5. *Cek Status Pesanan*:
   Gunakan perintah */cekpesanan* untuk mengetahui status pesananmu.

6. *Hubungi Admin*:
   Jika ada pertanyaan atau kendala, gunakan perintah */hubungics* untuk menghubungi admin.

🛍️ Selamat berbelanja di *${BOT_NAME}*! Jika ada pertanyaan, gunakan perintah */help*.`;
  await reply(tutorialMessage);
  break;
